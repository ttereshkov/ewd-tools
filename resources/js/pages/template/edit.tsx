...
export default function TemplateEdit({ template, aspects }: Props) {
    ...
    useEffect(() => {
        setData(
            ...
            selectedAspectIds
                .map((id) => {
                    const existing = data.selected_aspects.find((a) => a.id === id);
                    return existing || { id, weight: 0 };
                })
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit Template ${template.latest_template_version.name}`} />
            <div className="py-6 md:py-12">
                <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:max-w-7xl lg:px-8">
                    <Card>
                        <CardHeader className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                            <div>
                                <CardTitle className="text-lg font-bold md:text-2xl">Edit Template {template.latest_template_version.name}</CardTitle>
                                <p className="text-sm text-gray-600 dark:text-gray-400">
                                    Ubah judul, deskripsi, aspek, bobot, dan aturan visibilitas template.
                                </p>
                            </div>
